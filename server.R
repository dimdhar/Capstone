...
### Github repo : https://github.com/ddhar/capstone
...
Predict <- function(x) {
  ...
  if (length(xs)>= 3) {
    ...
    else {DisplayMessage <<- "Next word is predicted using 4-gram."; head(quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4],1)}
  }
  else if (length(xs) == 2){
    xs <- tail(xs,2)
    ...
    else {DisplayMessage<<- "Next word is predicted using 3-gram."; head(trigram[trigram$unigram == xs[1] & trigram$bigram == xs[2], 3],1)}
  }
  else if (length(xs) == 1){
    xs <- tail(xs,1)
    if (identical(character(0),head(bigram[bigram$unigram == xs[1], 2],1))) {DisplayMessage<<-"No match found. Most common word 'the' is returned."; head("the",1)}
    else {DisplayMessage <<- "Next word is predicted using 2-gram."; head(bigram[bigram$unigram == xs[1],2],1)}
  }
}
...
shinyServer(function(input, output) {
  output$prediction <- renderPrint({
    ...
    output$text2 <- renderText({DisplayMessage})
    result
  });
